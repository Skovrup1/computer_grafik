 ...
 *                            false otherwise
 * \param NeedsUpdate - true if the window needs to be updated - keypress or
 * window resize, false otherwise
 */
int method = 1;
int xmin = -9;
int xmax = 9;
...
std::string ErrorMessage(GLenum ErrorCode, bool Verbose = false) {
    ...
    case GL_NO_ERROR:
        errMessage << "no error";
        if (Verbose) {
            errMessage << "\nGlfwWindow: Error Code " << ErrorCode << ":"
                       << std::endl
    ...
    case GL_OUT_OF_MEMORY:
        ...
        if (Verbose) {
            ...
            errMessage << "\n\tThe state of the GL is undefined, "
                       << "except for the state of the error flags, after this "
                          "error is recorded.";
        }
        break;
    ...
    case GL_STACK_OVERFLOW:
        ...
        break;
    default:
        errMessage << "GlfwWindow: Error Code " << ErrorCode
                   << ": Unknown Error Code";
    }
 ...
 * \param BeQuiet - if true no message is output, else an error message is
 * output.
 */
 ...
 * \return A std::vector which contains the coordinates of the start and stop
 * points of a straight line.
 */
std::vector<glm::vec3> GenerateTestLine(int xstart, int ystart, int xstop,
                                        int ystop) {
...
/**
 * Scanconverts a straight line, i.e. computes the pixels that represents the
 * approximated straight line.
 * \param x1 - the x-coordinate of the start point.
 * \param y1 - the y-coordinate of the start point.
 * \param x2 - the x-coordinate of the end point.
 * \param y2 - the y-coordinate of the end point.
 * \return A std::vector which contains the coordinates of the pixels of a
 * straight line.
 */
...
void ResizeCallback(GLFWwindow *Window, int width, int height) {
    // std::cout << "-->ResizeCallback(GLFWwindow* Window, int width, int
    // height)" << std::endl; std::cout << "   width = " << width << ", height =
    // " << height << std::endl;

    // Set the point size - make the size of the dot be a little smaller than
...
/**
 * Change the coordinates of a vertex depending on the arrow key and the
 * modifier key
 * \param x - the x-coordinate of the vertex
 * \param y - the y-coordinate of the vertex
 * \param key - the arrow key that was pressed
 * \return x - there is no actual return value but the x-parameter is modified
 * according to the key-value
 * \return y - there is no actual return value but the y-parameter is modified
 * according to the key-value
 */
...
int main() {
    try {
        ...
        GLenum err = glewInit();
        // note: added condition to ignore GLEW_ERROR_NO_GLX_DISPLAY error to
        // make it work with wayland
        if (err != GLEW_OK && err != GLEW_ERROR_NO_GLX_DISPLAY) {
            std::ostringstream errmess;
            errmess << "GlfwWindow::Initialize(): "
                    << "GLEW failed to initialize: " << glewGetErrorString(err)
                    << ", (" << err << ") \n"
                    << "Status, Using GLEW version: "
        ...
        glDepthFunc(GL_LEQUAL); // You might change this when the
                                // transformations are in place!
        ...
        if (vertexprogID == 0) {
            std::ostringstream errormessage;
            errormessage << "The vertex program could not be created"
                         << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }
        ...
        if (!vertexsuccess) {
            ...
            std::ostringstream errormessage;
            errormessage
                << "Error compiling vertex program in file: vertexscale.vert: "
                << InfoLog << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }

        // Read and Compile the fragment program linefragment.frag
        std::string linefragmentprogram =
            Read(shader_path + "linefragment.frag");
        GLuint linefragmentprogID = glCreateShader(GL_FRAGMENT_SHADER);
        if (linefragmentprogID == 0) {
            std::ostringstream errormessage;
            errormessage << "The fragment program could not be created"
                         << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }
        ...
        glShaderSource(linefragmentprogID, 1, linefragmentprogramsrc,
                       linefragmentlengths);
        glCompileShader(linefragmentprogID);

        GLint linefragmentsuccess = 0;
        glGetShaderiv(linefragmentprogID, GL_COMPILE_STATUS,
                      &linefragmentsuccess);

        if (!linefragmentsuccess) {
            GLchar InfoLog[1024] = {0};
            glGetShaderInfoLog(linefragmentprogID, sizeof(InfoLog), NULL,
                               InfoLog);

            std::ostringstream errormessage;
            errormessage << "Error compiling fragment program in file: "
                            "linefragment.frag: "
                         << InfoLog << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }

        // Create a lineshader program and Link it with the vertex and
        // linefragment programs
        ...
        if (!lineprogramsuccess) {
            ...
            std::ostringstream errormessage;
            errormessage << "Error linking shaderprogram: " << InfoLog
                         << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }
        ...
        glShaderSource(dotfragmentprogID, 1, dotfragmentprogramsrc,
                       fragmentlengths);
        glCompileShader(dotfragmentprogID);

        GLint dotfragmentsuccess = 0;
        glGetShaderiv(dotfragmentprogID, GL_COMPILE_STATUS,
                      &dotfragmentsuccess);

        if (!dotfragmentsuccess) {
            GLchar InfoLog[1024] = {0};
            glGetShaderInfoLog(dotfragmentprogID, sizeof(InfoLog), NULL,
                               InfoLog);
        ...
        // Create dotshader program and Link it with the vertex and dotfragment
        // programs
        ...
        // Make a VertexArrayObject - it is used by the VertexArrayBuffer, and
        // it must be declared!
        GLuint GridVertexArrayID;
        glGenVertexArrays(1, &GridVertexArrayID);
        glBindVertexArray(GridVertexArrayID);

        // Make a GridvertexbufferObject - it uses the previous
        // VertexArrayBuffer!
        ...
        if (GridLines.size() > 0) {
            glBufferData(GL_ARRAY_BUFFER, GridLines.size() * sizeof(float) * 3,
                         &(GridLines[0][0]), GL_STATIC_DRAW);
        }

        // Validate the grid shader program
        GLint gridvalidationsuccess = 0;
        glValidateProgram(lineshaderID);
        glGetProgramiv(lineshaderID, GL_VALIDATE_STATUS,
                       &gridvalidationsuccess);
        if (!gridvalidationsuccess) {
            GLchar InfoLog[1024] = {0};
            glGetProgramInfoLog(lineshaderID, sizeof(InfoLog), NULL, InfoLog);

            std::ostringstream errormessage;
            errormessage << "Error validating the line shader program failed: "
                         << InfoLog << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }
        ...
        // GLuint linevertexpointsize = glGetUniformLocation(lineshaderID,
        // "PointSize");
        GLuint linefragmentcolor = glGetUniformLocation(lineshaderID, "Color");

        // Initialize grid Attributes
        GLuint linearvertexattribute =
            glGetAttribLocation(lineshaderID, "VertexPosition");
        glVertexAttribPointer(linearvertexattribute, 3, GL_FLOAT, GL_FALSE, 0,
                              0);
        ...
        if (TestLine.size() > 0) {
            glBufferData(GL_ARRAY_BUFFER, TestLine.size() * sizeof(float) * 3,
                         &(TestLine[0][0]), GL_STATIC_DRAW);
        }

        // Initialize grid Attributes
        GLuint testlineattribute =
            glGetAttribLocation(lineshaderID, "VertexPosition");
        ...
        if (LinePixels.size() > 0) {
            glBufferData(GL_ARRAY_BUFFER, LinePixels.size() * sizeof(float) * 3,
                         &(LinePixels[0][0]), GL_STATIC_DRAW);
        }
        ...
        if (!dotvalidationsuccess) {
            ...
            std::ostringstream errormessage;
            errormessage << "Error validating the dot shader program failed: "
                         << InfoLog << std::endl;
            throw std::runtime_error(errormessage.str().c_str());
        }
        ...
        GLuint dotvertexpointsize =
            glGetUniformLocation(dotshaderID, "PointSize");
        GLuint dotfragmentcolor = glGetUniformLocation(dotshaderID, "Color");

        // Initialize dot Attributes
        GLuint dotvertexattribute =
            glGetAttribLocation(dotshaderID, "VertexPosition");
        glVertexAttribPointer(dotvertexattribute, 3, GL_FLOAT, GL_FALSE, 0, 0);

        // Unbind the vertex array
        glBindVertexArray(0);

        // Set the point size - make the size of the dot be a little smaller
        // than the minimum distance between the grid lines
        ...
        std::cout << "*********************************************************"
                     "*************"
                  << std::endl;
        std::cout << "* Use the arrow keys to manipulate the end point of the "
                     "line         *"
                  << std::endl;
        std::cout << "* Use the shift arrow keys to manipulate the start point "
                     "of the line *"
                  << std::endl;
        std::cout << "*                                                        "
                     "            *"
                  << std::endl;
        std::cout << "* Press 1 - use a Function to do scan-conversion.        "
                     "            *"
                  << std::endl;
        std::cout << "* Press 2 - use a LineRasterizer to do scan-conversion.  "
                     "            *"
                  << std::endl;
        std::cout << "*                                                        "
                     "            *"
                  << std::endl;
        std::cout << "* The Window can be resized using the mouse              "
                     "            *"
                  << std::endl;
        std::cout << "*                                                        "
                     "            *"
            ...
            try {
                if (NeedsUpdate) {
                    ...
                    if (CoordinatesChanged) {
                        TestLine =
                            GenerateTestLine(xstart, ystart, xstop, ystop);
                        glBindBuffer(GL_ARRAY_BUFFER, testlinebuffer);
                        if (TestLine.size() > 0) {
                            glBufferData(GL_ARRAY_BUFFER,
                                         TestLine.size() * sizeof(float) * 3,
                                         &(TestLine[0][0]), GL_STATIC_DRAW);
                        }
                    }
                    if (TestLine.size() > 0) {
                    ...
                    if (CoordinatesChanged) {
                        LinePixels =
                            GenerateLinePixels(xstart, ystart, xstop, ystop);
                        glBindBuffer(GL_ARRAY_BUFFER, dotvertexbuffer);
                        if (LinePixels.size() > 0) {
                            glBufferData(GL_ARRAY_BUFFER,
                                         LinePixels.size() * sizeof(float) * 3,
                                         &(LinePixels[0][0]), GL_STATIC_DRAW);
                        }
                    }
                    if (LinePixels.size() > 0) {
